    ...
    class RepairSystemPatch
    {
            ...
            [HarmonyArgument(2)] byte amount)
        {
            ...
            if (!Options.MadmateCanFixComms.GetBool() && //Madmateがコミュサボを直せる設定がオフ
                ...
                return false;
            if (player.Is(CustomRoles.Sheriff) || player.Is(CustomRoles.Arsonist) || player.Is(CustomRoles.PlagueBearer) || player.Is(CustomRoles.Pestilence) || (player.Is(CustomRoles.Jackal) && !Options.JackalCanUseSabotage.GetBool()))
            {
